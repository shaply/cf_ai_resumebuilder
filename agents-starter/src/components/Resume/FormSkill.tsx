...
interface FormSkillProps extends SkillFormData {
  onUpdate?: (updatedItem: ResumeItem) => void;
  mode?: 'add' | 'edit';
}

export function FormSkill(props: FormSkillProps) {
  ...
  const handleSubmit = async (e: React.FormEvent, saveAndAddAnother = false) => {
    ...
    try {
      ...
      if (result.success) {
        const successMessage = isEdit ? "Skill updated successfully!" : "Skill saved successfully!";
  ...
  return (
    <Card className="p-6">
      <h3 className="text-lg font-semibold mb-4">{mode === 'edit' ? 'Edit Skill' : 'Skill'}</h3>
        ...
        <>
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Any additional context about this skill (proficiency level, years of experience, etc.)"
            />
          </div>
        </>
