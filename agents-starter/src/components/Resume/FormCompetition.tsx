...
interface FormCompetitionProps extends CompetitionFormData {
  onUpdate?: (updatedItem: ResumeItem) => void;
  mode?: 'add' | 'edit';
}

export function FormCompetition(props: FormCompetitionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent, saveAndAddAnother = false) => {
    ...
    try {
      ...
      if (result.success) {
        const successMessage = isEdit ? "Competition updated successfully!" : "Competition saved successfully!";
        ...
        setMessage({ type: "error", text: result.error || "Failed to save competition." });
      }
    } catch (error) {
  ...
  return (
    <Card className="p-6">
      <h3 className="text-lg font-semibold mb-4">{mode === 'edit' ? 'Edit Competition' : 'Competition'}</h3>
        ...
        <>
          <div>
            ...
            <input
              ...
              placeholder="e.g., Google Code Jam, ACM ICPC"
              required
            />
          </div>
          ...
          <div>
            <label className="block text-sm font-medium mb-2">
              Resume Item Name (Optional)
            </label>
            <input
              type="text"
              name="resumeItemName"
              value={formData.resumeItemName}
              onChange={handleInputChange}
              className="w-full p-3 border border-neutral-300 dark:border-neutral-700 rounded-lg bg-white dark:bg-neutral-900"
              placeholder="Custom name for resume (if different from competition name)"
            />
          </div>
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Describe the competition, your solution/approach, team size, etc."
            />
          </div>
        </>
